...
export function HeroBalanceCard({ summary }: { summary: DashboardSummary }) {
  ...
  return (
    ...
    >
      ...
      <p className="font-serif text-[38px] font-bold leading-none text-hero-text mb-6">
      ...
      <div className="grid grid-cols-3 gap-2 border-t border-white/8 pt-4">
        <div>
          ...
          <p className="font-sans text-sm font-semibold text-hero-income">+{formatCurrency(summary.entrate)}</p>
        </div>
        <div className="border-x border-white/8 px-2">
          <p className="font-sans text-[10px] text-white/40 mb-0.5">Uscite</p>
          <p className="font-sans text-sm font-semibold text-hero-expense">-{formatCurrency(summary.uscite)}</p>
        </div>
        <div className="pl-2">
