  ...
  return (
    <>
      <button
        type="button"
        onClick={() => onSelect("")}
        className={`flex w-full items-center px-4 py-3 text-sm hover:bg-surface-hover ${
          !value ? "text-foreground" : "text-text-2"
        }`}
      >
        Nessuna categoria
      </button>
        ...
        filteredFlat.length === 0 ? (
          ...
          filteredFlat.map((cat) => (
            <button
              ...
              className={`flex w-full items-center gap-3 px-4 py-3 text-sm hover:bg-surface-hover ${
                value === cat.id ? "text-foreground" : "text-text-1"
              }`}
            >
              <span
...
export function CategoryCombobox({ categories, txType, value, onChange }: CategoryComboboxProps) {
  ...
  const trigger = (
    <button
      ...
      className="flex h-10 w-full items-center justify-between rounded-md border border-border-subtle bg-surface-2 px-3 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    >
      {selected ? (
  ...
  if (isMobile) {
    return (
      <div>
        {trigger}
        {open && (
          <div className="fixed inset-0 z-[300] flex flex-col bg-surface-1">
            {/* Header */}
            <div className="flex shrink-0 items-center gap-3 border-b border-border-subtle px-4 py-3">
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="flex h-9 w-9 items-center justify-center rounded-lg text-text-2 hover:bg-surface-hover hover:text-foreground"
                aria-label="Chiudi"
              >
            ...
            <div className="flex shrink-0 items-center gap-2 border-b border-border-subtle px-4 py-3">
              ...
              <input
                ...
                className="flex-1 bg-transparent text-sm text-foreground outline-none placeholder:text-text-3"
              />
            </div>
