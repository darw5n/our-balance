    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-50">
      ...
      <header className="border-b border-white/10 bg-zinc-950/60 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between px-4">
          ...
          {/* Right: add button (desktop) + secondary menu */}
          <div className="flex items-center gap-2">
            {/* Aggiungi — desktop only */}
            <button
              onClick={() => setAddOpen(true)}
              className="hidden items-center gap-1.5 rounded-md border border-white/10 px-3 py-1.5 text-xs font-medium text-zinc-400 transition-colors hover:bg-zinc-800/50 hover:text-zinc-200 md:flex"
            >
            ...
            <div className="relative" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full z-50 mt-2 w-52 rounded-xl border border-white/10 bg-zinc-900 p-1.5 shadow-xl">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-zinc-950/95 backdrop-blur md:hidden">
        <div className="flex h-16 items-stretch">
          ...
          <button
            onClick={() => setAddOpen(true)}
            className="flex flex-1 flex-col items-center justify-center gap-1 text-zinc-500 transition-colors hover:text-zinc-300"
            aria-label="Aggiungi transazione"
          >
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-500">
              <Plus className="h-4 w-4 stroke-[2.5] text-zinc-950" />
            </div>
          </button>
        </div>
      </nav>
